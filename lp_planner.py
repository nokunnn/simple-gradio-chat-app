...
def generate_lp_planning(product_theme, csv_insights=None, svg_path=None):
    ...
    try:
        ...
        if csv_insights:
            ...
            職種別の選択傾向分析:
            {csv_insights.get('cross_tabs_insights', '情報なし')}

            サンプルデータ:
            {csv_insights.get('sample_data_json', '{}')}

            上記のCSVデータの分析結果（特に職種別の選択傾向分析）を参考にして、以下の3つの観点から分析を行ってください:

            1. ターゲットの分析: このサービス/商品の理想的な法人顧客はどのような企業か、どのような課題を持っているのか
               - CSVデータで見られた職種別の特徴的な傾向を活用し、ターゲットセグメントごとの特性を具体的に分析してください
               - 最も反応が良かった選択肢と職種の組み合わせに注目し、ターゲットを具体化してください
               
            2. 訴求軸の検討: 商品/サービスの最も魅力的な特徴と、それによって解決される顧客の課題
               - 職種ごとの選択傾向の違いを考慮し、セグメント別の訴求ポイントを検討してください
               - CSVデータで明らかになった各職種の関心事や課題に対応する解決策を提案してください
               
            3. 訴求シナリオの検討: LPで情報を伝達する最適な順序、各セクションで伝えるべき内容
               - 職種別のニーズの違いを考慮した情報設計を行ってください
               - 重要なセグメントに対して、クロス集計で明らかになった特徴的な選択傾向に基づいたコンテンツを提案してください

            分析内容は必ず以下の形式で出力してください:
            ```
            ## ターゲットの分析
            （ターゲット分析の内容 - CSVデータの職種別選択傾向を必ず活用してください）
            ...
            各セクションは具体的で、詳細な分析を含め、長さは合計で1000字程度にしてください。
            特にターゲット分析では、CSVデータから得られた職種別の選択傾向の洞察を具体的に言及してください。
            """
